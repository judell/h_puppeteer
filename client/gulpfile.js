...
gulp.task('build-html', function() {
  ...
  var config = {
    ...
    oauthClientId: '472a4926-a306-11e7-8b1b-eb37720ea07c',
    oauthEnabled: true,
...
function generateBootScript(manifest) {
  ...
  */
  ...
  const defaultSidebarAppUrl = `https://sci.jonudell.info/hypothesis/${version}/build/app.html`;

  const defaultAssetRoot = `https://sci.jonudell.info/hypothesis/${version}/build/`;

  /*
  if (process.env.NODE_ENV === 'production') {
    defaultAssetRoot = `https://sci.jonudell.info/hypothesis/${version}/`;
  } else {
...
gulp.task('serve-live-reload', function () {
  ...
  const scheme = useSsl ? 'https' : 'http';
  liveReloadServer = new LiveReloadServer(2999, {
